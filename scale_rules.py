...
# reject date-like denominators; allow common scales including 4.33
ALLOWED_FRACTION_DENOMS = {4.0, 4.3, 4.33, 5.0, 6.0, 10.0, 20.0, 100.0}
...
def conv_four_point_three(value: float) -> Optional[float]: return conv_linear(value, 4.33)
...
def _detect_by_value(val: float) -> Optional[str]:
    ...
    if 4.0 < v <= 4.33:   return 'four_point_three'
    if 4.33 < v <= 5.0:   return 'five_point'
...
# accept comma decimals inside fractions
_NUM = r'(\d+(?:[.,]\d+)?)'
...
def apply_conversion(
    ...
    *,
    ...
    # institution hint (from CLI)
    if not lab and uname:
        for k, v in INSTITUTION_HINTS.items():
            if k in uname:
                lab = _norm_lower_or_none(v); break

    # country defaults (very light touch; can be overridden)
    COUNTRY_HINTS.update({
        'uk':'percent_100','united kingdom':'percent_100',
        'australia':'percent_100',
        'germany':'german_reversed_5','de':'german_reversed_5',
        'poland':'polish_55',
        'france':'twenty_point',
        'china':'percent_100',
        'india':'ten_point',
        'canada':'four_point_three',
    })
    if not lab and ch:
